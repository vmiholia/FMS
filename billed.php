...
<body>
				...
				<nav>
				<div class="col-md-5">
				<legend align="left">View Transactions</legend>
		...
		if($row['month']=="Apr-May"||$row['month']=="Jun-July"||$row['month']=="Aug-Sept"||$row['month']=="Oct-Nov"||$row['month']=="Dec-Jan"||$row['month']=="Feb-Mar"){ 
			...
			<?php }} } ?>
				...
				<div style="text-align:center">  	
				 <input type="submit" style="margin: 0 auto;" name="inmon" value="Search">
				</div>
				</div>
		...
		<tr class="warning no-result">
			<td colspan="4"><i class="fa fa-warning"></i> No result</td>
						...
						elseif($mahina==12)
						{
							...
							$qry = mysql_query("SELECT * FROM payment_record WHERE month = '".$month_name."'")or die(mysql_error());
							...
							//$stat = "$rec[status]";
								echo "
								<tr>
								<td scope='row'>LFYV/$rec[transaction_id]</td>
								<td>$rec[Date]</td>
								<td>$name</td>
								<td>$fname</td>
								<td>$class</td>
